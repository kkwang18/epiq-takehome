...
- `run_worker`'s main loop in `src/content_intake/worker/worker.py` (line 198 and surrounding per-item processing) has no exception handling around `process_item` — an unhandled exception (e.g. an unreadable corpus file) would crash the whole worker process rather than being caught and logged. This gap was not exercised during M1 testing but is a known, unaddressed limitation flagged during code review.
